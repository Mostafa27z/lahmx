...
@section('title', 'تعديل القسم - لوحة التحكم')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-3xl font-bold text-gray-900">تعديل القسم 📁</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            @if($category->image)
                <div>
                    <span class="block text-sm font-bold text-gray-700 mb-2">الصورة الحالية</span>
                    <img src="{{ asset('storage/' . $category->image) }}" alt="" class="w-32 h-32 object-cover rounded-xl border border-gray-200">
                </div>
            @endif

            <!-- Image Upload -->
            <div>
                <label for="image" class="block text-sm font-bold text-gray-700 mb-2">تحديث صورة القسم</label>
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="is_active" class="font-bold text-gray-700">تفعيل القسم بالموقع</label>
            </div>
